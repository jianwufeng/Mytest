package jian.com.tracking.kafka.constants;

import java.io.Serializable;
import java.util.Date;

import jian.com.tracking.SyncPojo;
import jian.com.tracking.util.TrackingMessageUtil;
...
public class TrackingMessageInfo implements SyncPojo, Serializable {
    ...
    public String toString() {
        return "" + (logEvent == null ? "1" : logEvent) + '|' + TrackingMessageUtil.escapse(resolution) + '|' + TrackingMessageUtil.escapse(operatorSystem)
                + '|' + TrackingMessageUtil.escapse(browser) + '|' + TrackingMessageUtil.escapse(colorDepth) + '|' + TrackingMessageUtil.escapse(flashVersion)
                + '|' + (javaEnabled == null ? "" : javaEnabled) + '|' + TrackingMessageUtil.escapse(device) + '|' + TrackingMessageUtil.escapse(localIp) + '|'
                + TrackingMessageUtil.escapse(trackerCode) + '|' + TrackingMessageUtil.escapse(categroy) + '|' + TrackingMessageUtil.escapse(theme) + '|'
                + (userId == null ? "" : userId) + '|' + TrackingMessageUtil.escapse(ip) + '|' + TrackingMessageUtil.escapse(region) + '|'
                + TrackingMessageUtil.escapse(sessionId) + '|' + (trackTime == null ? "" : DateUtils.format(trackTime, "yyyyMMddHHmmss")) + '|'
                + TrackingMessageUtil.escapse(guid) + '|' + TrackingMessageUtil.escapse(referrer) + '|' + TrackingMessageUtil.escapse(url) + '|'
